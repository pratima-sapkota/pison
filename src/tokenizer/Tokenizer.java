...
public class Tokenizer {
    ...
    private int getStringToken(int pos) {
        while (pos < mChunk.length) {
            int escapeCnt = 0;
            while (pos < mChunk.length && mChunk[pos] == '\\') {
                ++escapeCnt;
                ++pos;
            }
            if (pos < mChunk.length && mChunk[pos] == '"') {
                ++pos;
                if (escapeCnt % 2 == 0) {
                    mCurTknType = STRING;
                    mNextTknPos = pos;
                ...
                case 'l': {
                    if (pos + 1 < mChunk.length &&
                        (mChunk[pos + 1] == 'l' ||
                         ...
                         mChunk[pos + 1] == '}')) {
                        ++pos;
                        if (pos < mChunk.length && mChunk[pos] == 'l')
                            ++pos;
                        mCurTknType = PRI;
                        mNextTknPos = pos;
                        return TRUE;
